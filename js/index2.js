/*
 *
 * @author start Rosenstrom
 * http://www.start.net
 * https://twitter.com/start
 * http://www.devstate.net
 * Sources for this can be found at:
 * https://github.com/start/startNetCommonJS
 */
...
(function (window){
  ...
  window.start = window.start || start;
	...
	start.MathUtil.PI_180 = Math.PI/180;
	start.MathUtil.ONE80_PI = 180/Math.PI;
	
	//precalculations for values of 90, 270 and 360 in radians
	start.MathUtil.PI2 = Math.PI*2;
	start.MathUtil.HALF_PI = Math.PI/2;


	//return number between 1 and 0
	start.MathUtil.normalize = function(value, minimum, maximum){
		return (value - minimum) / (maximum - minimum);
	};

	//map normalized number to values
	start.MathUtil.interpolate = function(normValue, minimum, maximum){
		return minimum + (maximum - minimum) * normValue;
	};

	//map a value from one set to another
	start.MathUtil.map = function(value, min1, max1, min2, max2){
		return start.MathUtil.interpolate( start.MathUtil.normalize(value, min1, max1), min2, max2);
	};

	start.MathUtil.getRandomNumberInRange = function(min, max){
		return min + Math.random() * (max - min);
	};
	
	start.MathUtil.getRandomIntegerInRange = function(min, max){
		return Math.round(start.MathUtil.getRandomNumberInRange(min, max));
	};
...
(function (window){
    ...
    start.Geom.Point = function (x,y){
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(y) ? 0 : y;
    };

    start.Geom.Point.prototype.clone = function(){
        return new start.Geom.Point(this.x,this.y);
    };

    start.Geom.Point.prototype.update = function(x, y){
        this.x = isNaN(x) ? this.x : x;
        this.y = isNaN(y) ? this.y : y;
    };

    start.Geom.Point.prototype.equals = function(point){
        return this.x==point.x && this.y==point.y;
    };

    start.Geom.Point.prototype.toString = function(){
	...
	start.Geom.Rectangle = function (x, y, width, height){
		this.update(x, y, width, height);
	};
	
	start.Geom.Rectangle.prototype.update = function(x, y, width, height){
		this.x = isNaN(x) ? 0 : x;
		this.y = isNaN(y) ? 0 : y;
	...
	start.Geom.Rectangle.prototype.getRight = function(){
		return this.x + this.width;
	};
	
	start.Geom.Rectangle.prototype.getBottom = function(){
		return this.y + this.height;
	};

    start.Geom.Rectangle.prototype.getCenterX = function(){
        return this.x + this.width/2;
    };

    start.Geom.Rectangle.prototype.getCenterY = function(){
        return this.y + this.height/2;
    };

    start.Geom.Rectangle.prototype.containsPoint = function(x, y){
        return x >= this.x && y >= this.y && x <= this.getRight() && y <= this.getBottom();
    };

	
	start.Geom.Rectangle.prototype.clone = function(){
		return new start.Geom.Rectangle(this.x, this.y, this.width, this.height);
	};
	
	start.Geom.Rectangle.prototype.toString = function(){
...
/**
 * Created by start on 27-1-14.
 * has a dependecy on start.Geom
 * has a dependecy on start.BitmapUtil
 */
...
(function (window){
    ...
    start.CanvasTextUtil = {};

    //returns the biggest font size that best fits into rect
    start.CanvasTextUtil.getFontSizeForRect = function(string, fontProps, rect, canvas, fillStyle){
        if(!canvas){
    ...
    start.CanvasTextProperties = function(fontWeight, fontStyle, fontSize, fontFace){
    ...
    start.CanvasTextProperties.NORMAL = "normal";
    start.CanvasTextProperties.BOLD = "bold";
    start.CanvasTextProperties.BOLDER = "bolder";
    start.CanvasTextProperties.LIGHTER = "lighter";

    start.CanvasTextProperties.ITALIC = "italic";
    start.CanvasTextProperties.OBLIQUE = "oblique";


    start.CanvasTextProperties.prototype.setFontWeight = function(fontWeight){
        switch (fontWeight){
            case start.CanvasTextProperties.NORMAL:
            case start.CanvasTextProperties.BOLD:
            case start.CanvasTextProperties.BOLDER:
            case start.CanvasTextProperties.LIGHTER:
                this.fontWeight = fontWeight;
                break;
            default:
                this.fontWeight = start.CanvasTextProperties.NORMAL;
        }
    };

    start.CanvasTextProperties.prototype.setFontStyle = function(fontStyle){
        switch (fontStyle){
            case start.CanvasTextProperties.NORMAL:
            case start.CanvasTextProperties.ITALIC:
            case start.CanvasTextProperties.OBLIQUE:
                this.fontStyle = fontStyle;
                break;
            default:
                this.fontStyle = start.CanvasTextProperties.NORMAL;
        }
    };

    start.CanvasTextProperties.prototype.setFontSize = function(fontSize){
    ...
    start.CanvasTextProperties.prototype.clone = function(){
        return new start.CanvasTextProperties(this.fontWeight, this.fontStyle, this.fontSize, this.fontFace);
    };

    start.CanvasTextProperties.prototype.getFontString = function(){
...
function commitResize(){
    ...
    htmlBounds = new start.Geom.Rectangle(0,0, getWidth(canvasContainer) , getHeight(canvasContainer));
    ...
    bounds = new start.Geom.Rectangle(0,0, canvas.width, canvas.height);
    context.clearRect(0,0,canvas.width, canvas.height);
...
//========================
    ...
    var scrollSpeed = 3;
    var score;
    var fontProperties = new start.CanvasTextProperties(start.CanvasTextProperties.BOLD, null, 100);
    ...
    function createLogo(logoText, logoCanvas, logoCanvassBG){
        ...
        var textRect = new start.Geom.Rectangle(0, 0, logoCanvas.width * .8, logoCanvas.height);
        var logoFontProps = fontProperties.clone();
        logoFontProps.fontSize = start.CanvasTextUtil.getFontSizeForRect(logoText, fontProperties, textRect);
    ...
    var collisionPoint = new start.Geom.Point();
    var birdPoints = [];
    ...
    var characters;
    var birdFontProperties = new start.CanvasTextProperties(start.CanvasTextProperties.BOLD, null, 50);
    ...
    function createTubes(){
        ...
        for(var i = 0; i < totalTubes; i++){
            ...
            tubes[i].topRect = new start.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            tubes[i].bottomRect = new start.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            renderTube(tubes[i]);
        }
        currentTube = tubes[0];
    }
...
function createCityGraphic(){
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minCloudRadius){
        cgContext.beginPath();
        cgContext.arc( iterator , maxCloudRadius, start.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius), 0, start.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    }
    ...
    for(iterator=0; iterator<canvas.width; iterator+=(houseWidth+8)){
        ...
        houseHeight = start.MathUtil.getRandomNumberInRange(cityGraphicHeight *.5 , cityGraphicHeight - maxCloudRadius *.8);
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minTreeRadius){
        cgContext.beginPath();
        radius = start.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius)
        cgContext.arc( iterator , cityGraphicHeight, radius, 0, start.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    ...
    var sand = "#dcd795";
    var groundGraphicRect = new start.Geom.Rectangle();
